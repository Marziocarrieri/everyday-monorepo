...
import 'dart:ui';
import 'package:everyday_app/shared/utils/date_utils.dart' as shared_utils;
...
// ==========================================
// SCHERMATA PRINCIPALE (PERSONNEL)
// ==========================================
class PersonnelFridgeKeepingScreen extends StatefulWidget {
  const PersonnelFridgeKeepingScreen({super.key});

  @override
  State<PersonnelFridgeKeepingScreen> createState() => _PersonnelFridgeKeepingScreenState();
}

class _PersonnelFridgeKeepingScreenState extends State<PersonnelFridgeKeepingScreen> {
  ...
  Future<void> _showItemDetailModal(MockFridgeItem item) async {
    await showModalBottomSheet<bool>(
      ...
      builder: (_) => PersonnelFridgeItemDetailSheet(item: item),
    );
  }
...
// ==========================================
class PersonnelFridgeItemDetailSheet extends StatefulWidget {
  final MockFridgeItem item;
  const PersonnelFridgeItemDetailSheet({super.key, required this.item});

  @override
  State<PersonnelFridgeItemDetailSheet> createState() => _PersonnelFridgeItemDetailSheetState();
}

class _PersonnelFridgeItemDetailSheetState extends State<PersonnelFridgeItemDetailSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRRect(
      ...
      child: BackdropFilter(
        filter: ImageFilter.blur(sigmaX: 20.0, sigmaY: 20.0),
        child: Container(
          padding: EdgeInsets.only(left: 30, right: 30, top: 30, bottom: MediaQuery.of(context).viewInsets.bottom + 40),
          decoration: BoxDecoration(
            gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [themeColor.withValues(alpha: 0.15), Colors.white.withValues(alpha: 0.8)]),
            border: Border.all(color: Colors.white.withValues(alpha: 0.8), width: 1.5),
          ),
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Container(width: 50, height: 5, margin: const EdgeInsets.only(bottom: 30), decoration: BoxDecoration(color: themeColor.withValues(alpha: 0.4), borderRadius: BorderRadius.circular(10))),
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Expanded(
                    child: _isEditing 
                      ? TextField(controller: _nameController, style: GoogleFonts.poppins(fontSize: 24, fontWeight: FontWeight.w700, color: const Color(0xFF3D342C)), decoration: const InputDecoration(border: InputBorder.none))
                      : Text(widget.item.name, style: GoogleFonts.poppins(fontSize: 26, fontWeight: FontWeight.w800, color: const Color(0xFF3D342C), letterSpacing: -0.5)),
                  ),
                  GestureDetector(
                    onTap: () => setState(() => _isEditing = !_isEditing),
                    child: Container(
                      padding: const EdgeInsets.all(10),
                      decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.6), shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 1.5)),
                      child: Icon(_isEditing ? Icons.check_rounded : Icons.edit_rounded, color: themeColor, size: 20),
                    ),
                  ),
                ],
              ),
              ...
              Container(
                ...
                decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.6), borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.white, width: 1.5)),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Quantity', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: const Color(0xFF3D342C).withValues(alpha: 0.6))),
                    Row(
                      children: [
                        GestureDetector(
                          onTap: () {
                          ...
                          child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: themeColor.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(Icons.remove_rounded, color: themeColor, size: 20)),
                        ),
                        ...
                        SizedBox(
                          ...
                          child: TextField(
                            ...
                            style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w700, color: const Color(0xFF3D342C)),
                        ...
                        GestureDetector(
                          ...
                          child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: themeColor.withValues(alpha: 0.1), shape: BoxShape.circle), child: Icon(Icons.add_rounded, color: themeColor, size: 20)),
                        ),
                      ],
                    ),
                  ],
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: const Color(0xFFF28482).withValues(alpha: 0.15), borderRadius: BorderRadius.circular(24), border: Border.all(color: const Color(0xFFF28482).withValues(alpha: 0.3), width: 1.5)),
                  child: Center(child: Text('Consume / Delete', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w700, color: const Color(0xFFF28482)))),
                ),
              ),
            ],
          ),
