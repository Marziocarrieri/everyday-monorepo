...
// ==========================================
class PersonnelDailyTaskScreen extends StatefulWidget {
  final DateTime date;

  const PersonnelDailyTaskScreen({super.key, required this.date});

  @override
  State<PersonnelDailyTaskScreen> createState() => _PersonnelDailyTaskScreenState();
}

class _PersonnelDailyTaskScreenState extends State<PersonnelDailyTaskScreen> {
  late List<MockTask> _tasks;

  @override
  void initState() {
    super.initState();
    // Dati finti per il Personnel
    _tasks = [
      MockTask(
  ...
  void _simulateSaveNote(MockTask task, String note) { 
    setState(() => task.note = note); 
    _showSuccessSnackBar('Nota salvata'); 
  }
  ...
  Widget _buildHeader(String formattedDate) {
    ...
    return Row(
      ...
      children: [
        ...
        Column(
          children: [
            Text('My Tasks', style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.w800, color: themeColor)),
        ...
        // IL TASTO "+" È STATO RIMOSSO PER IL PERSONNEL
        // Metto un SizedBox della stessa larghezza del back button per mantenere i testi perfettamente centrati
...
// ==========================================
// COMPONENTE: SINGOLA CARD TASK (Senza Dismissible)
// ==========================================
class _TaskCard extends StatefulWidget {
  ...
  const _TaskCard({
    super.key, required this.task, required this.onSubtaskToggle, required this.onTaskToggle, required this.onSaveNote,
  });
...
class _TaskCardState extends State<_TaskCard> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      // RIMOSSO IL DISMISSIBLE (Il dipendente non può eliminare i task)
      // Usiamo solo lo Stack per la grafica
      child: Stack(
        clipBehavior: Clip.none,
        children: [
          // CONTENUTO ESPANSO (Sotto la card principale)
          if (_isExpanded)
            Container(
